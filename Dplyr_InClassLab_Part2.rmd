...
```{r}
houston_flights <- flights %>% select(origin, dest, gain) %>% filter(dest =="HOU")
houston_flights <- flights %>% select(origin, dest, gain) %>% filter(dest %in% c("HOU","IHA"))
houston_flights
```
...
flights %>%
  ...
  select(origin, year, month, day, flight, dest, dep_delay) %>%
...
```{r}
flights %>%
  summarise(airports = sort(unique(c(origin, dest)))) %>%
  pull(airports)
...
starwars%>% group_by(homeworld)%>% summarise(n_charaters = n())
...
starwars %>%
  ...
  select(name, height, sex, gender, homeworld)
```
...
starwars %>%
  filter(gender == "female", eye_color == "blue") %>%
  summarise(n = n())
